...
3. Save into this folder with a descriptive filename, update the `src` in the relevant `<img>` tag, and write accurate `alt` text describing what the photo *actually* shows, not an idealized or invented scene. Alt text should be verifiably true, both for accessibility and to avoid misrepresenting sourced content.
